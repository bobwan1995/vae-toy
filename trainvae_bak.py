...
test_dataset = CelebA(CELEB_PATH, transform=celeb_transform, download=False, split='valid')
...
def loss_function(recon_x, x, mu, log_var):
    # recon_loss = F.mse_loss(recon_x, x.view(-1, image_dim))
    # kl_div = -0.5 * torch.mean(1 + log_var - mu.pow(2) - log_var.exp())
    recon_loss = F.binary_cross_entropy(recon_x, x.view(-1, image_dim), reduction='sum')
...
def train(epoch):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        recon_loss, kl_div = loss_function(recon_batch, data, mu, log_var)
